...
edges <- read.csv("~/Documents/coding/R/Project/sna-coauthorship/edges.csv")
nodes <- read.csv("~/Documents/coding/R/Project/sna-coauthorship/nodes.csv")
id <- seq(1,593,1)
...
sim2 <- simulate(m7, MCMC.burnin = 3000, verbose = TRUE, seed = 9)
#model2 <- ergm(net ~ edges + nodematch("ARTICLE") + nodematch("ARTIPERYEAR"))
plot(summary(net ~ idegree(0:10)), type = "l", lty = 1, lwd = 2, xlab = "Degree", ylab = "Count")
lines(summary(sim2 ~ idegree(0:10)), lty = 2, lwd = 3)
...
set.edge.attribute(net2, "seat_net", edges3[,6])
...
m2<-ergm(net ~ edges + mutual ,burnin=20000,
...
m2_fast<-ergm(net ~ edges + mutual + nodematch("DURATIONr") +
                      nodematch("ARTICLES")  + edgecov(lag_mut),burnin=20000,
              MCMCsamplesize=70000,verbose=FALSE,seed=25,
